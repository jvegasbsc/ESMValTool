...
from toz_diagnostic import TOZDiagnostic # <==HERE!!!


def main(project_info):
	print('>>>>>>>> DUMMY_VAR_PROJECT_Ben.py is running! <<<<<<<<<<<<') # <==HERE!!!
	...
	for v in range(len(project_info['RUNTIME']['currDiag'].get_variables())):
		...
		for inc in range(len(project_info['MODELS'])):
			...
			if Mod_L1 == Ref_L1:
                ...
                # load the data
				#Diag.calc_mean()
				print(Diag.__dict__.keys())
				Diag.load_data()

				print("after loading data into Diag")
