...
def do_eval_table(varname, eval_expert, eval_reference):
    """
    Author  :  Arun Rana
    ...
    Inputs:
        ...
        - eval_reference: A dictionary the reference values (validation standards for that product)
                          keys must of course match the expert file!
                          Example:
                          {"Mean climatology":{"value":30,"unit":"years"},
                           "Trend":{"value":30,"unit":"years"},
                           "Variability":{"value":0.1,"unit":"NA"}}
    Outputs:
        - A *.png file that includes a two-row table: first row = header,
          second row = numbers from expert with green or red or white shading depending
...
#    path_report_out = work_dir + "/reports/sphinx/source"
    ...
    # Create a list. Each item of the list will be itself a list of strings, corresponding either to the 
    # headers or to the validation reference values
    contents = list()
    with open(eval_reference, 'rb') as csvfile:
        s = csv.reader(csvfile, delimiter = ",", skipinitialspace = True)
        for row in s:
            contents.append(row)
...
#            if counter_y == 0:
#                if not [l.strip().lower() for l in row] == [l.strip().lower() for l in contents[0]]:
#                    sys.exit("(do_eval_report) NO MATCH-UP BETWEEN HEADER NAMES IN REFERENCE AND PRODUCT CSV FILES. MAKE SURE NAMES AND ORDER MATCH UP.")
#            else:
#                contents.append(row)
#            counter_y += 1
    
    if not isinstance(eval_expert, dict):
        assert False, "wrong input type in eval"
    elif not np.all(np.sort(eval_expert.keys()) == np.sort(contents[0])):
        assert False, "wrong names in eval"
    else:
        data_contents=[]
        for key in contents[0]:
            if eval_expert[key]["unit"] is None:
                this_unit = ""
            elif eval_expert[key]["unit"] in ["1","unkown","no-unit"]:
                this_unit = ""
            else:
                this_unit = " " + eval_expert[key]["unit"]
            data_contents.append(str(eval_expert[key]["value"]) + this_unit)
        contents.append(data_contents)
    ...
    # TO BE IMPROVED WITH NEW BACK END
    if sum([1.0 * (len(contents[i])==nx) for i in range(len(contents))] ) != ny:
        sys.exit("(do_eval_report) STOP: uneven number of columns in reference file")
...
#    if not (os.path.exists(path_out)):
#      os.makedirs(path_out)
#    plt.savefig(path_out + "/" + "eval_requirements.png", dpi = 400)
#    plt.close(fig)
#
#
#    # Create *.rst report
...
#    file.write("=" * len(my_title) + "\n\n")
#
#    file.write(".. figure:: " + "../../../../plot_scripts/python/gcos/eval_requirements.png" + "\n"
#               "   :align:   center"   + "\n"
#               "   :width:   95%" + "\n"
#              )
#    file.close()
