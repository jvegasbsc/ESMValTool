...
def get_xclim(cube, percentile, doy_range, window_size=None):
    ...
    # shape (len(doy_range), ntimesteps)
    doymask = get_doy_mask(cube, doy_range, window_size)
    result = np.empty(doymask.shape[:1] + cube.shape[1:])
    print(result.shape)
    for n, doy in enumerate(doy_range):
        print(
            f"Reading data into memory for window around day-of-year {doy}")
        doy_data = cube[doymask[n, :], :, :].data.filled(np.nan)
        print("Calculating clim...")
        result[n, :, :] = parallel_apply_along_axis(
            np.nanpercentile, 0, doy_data, percentile)
    return result
...
def subtract_xclim(cube, percentile=None, refcube=None, window_size=None):
    ...
    try:
        iris.coord_categorisation.add_day_of_year(
            cube, 'time', name='day_of_year')
    except ValueError:
        pass
    xclim_array = get_xclim(cube, percentile, cube.coord(
        'day_of_year').points, window_size)
    # Now repeat
    if percentile >= 50:
...
#        event_cube = self._extremes_preprocessing(event_cube)
        ...
        for dat in ["severity", "magnitude", "duration"]:
            ...
            for reg, m in extremes_measures.items():
                loc_list.append(np.nanpercentile(
                    m[dat].data.compressed(), [5, 95]))
            ...
            for dat in ["time_series"]:
                ...
                if all(amplitude_time.data.mask) or all(extent_time.data.mask):
                    self.__logger__.warning(
                        "Extremes: spatially aggregated amplitude or extent are all masked! No lineplots produced.")
                else:
                    # plotting lineplots
                    filename = self.__plot_dir__ + os.sep + \
                        basic_filename + \
                    ...
                    ax = np.array([plt.subplot(gs[0:4, :]),
                                   plt.subplot(gs[4:8, :])])
                    plt.sca(ax[0])
                    iplt.plot(amplitude_time)
                    plt.ylabel(amplitude_time.long_name +
                               " [{}]".format(amplitude_time.units))
                    ...
                    plt.ylabel(extent_time.long_name +
                               " [{}]".format(extent_time.units))
        ...
        column_rename_dict = {'severity': 'severity [{0}]'.format(severity_av.units), 'magnitude': 'magnitude [{0}]'.format(
            magnitude_av.units), 'duration': 'duration [{0}]'.format(duration_av.units), 'extent': 'extent [{0}]'.format(extent.units)}
        ...
        savename = self.__work_dir__ + os.sep + \
            basic_filename + "_extreme_event_metrics."
        savename_csv = savename + "csv"
        savename_html = savename + "html"

        # Save to csv (keeping full precision)
        self.__logger__.info(
            "Saving metric csv-table as: {0}".format(savename_csv))
